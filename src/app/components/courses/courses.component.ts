...
export class CoursesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Subscribe to search subject with debouncing and filtering
    this.searchSubject$
      .pipe(
        debounceTime(300), // Wait 300ms after user stops typing
        distinctUntilChanged(), // Only if search text changed
        filter(searchText => searchText.trim().length >= 3 || searchText.trim().length === 0), // Filter for >= 3 chars or empty
   ...
   * Emits the search text to the searchSubject$ for processing
   * @param searchText - The search text from the input
   */
  ...
  /**
   * Handle search button click (DEPRECATED - kept for backward compatibility)
   * Makes server-side request using /search endpoint
   */
  onSearch(): void {
